...
describe('Unit | Service | workflow actions', function () {
  ...
  it('creates RemoveAtmLambdaAction instance', function () {
    const service = this.subject();

    const atmLambda = {};
    const atmInventory = {};
    const action = service.createRemoveAtmLambdaAction({
      atmLambda,
      atmInventory,
    });

    expect(action).to.be.instanceOf(RemoveAtmLambdaAction);
    expect(get(action, 'atmLambda')).to.equal(atmLambda);
    expect(get(action, 'atmInventory')).to.equal(atmInventory);
  });

...
describe('Integration | Component | content workflows functions', function () {
  setupComponentTest('content-workflows-functions', {
    integration: true,
  });
  ...
  it('has class "content-workflows-functions"', function () {
    this.render(hbs `{{content-workflows-functions}}`);

    expect(this.$().children()).to.have.class('content-workflows-functions')
  ...
  it('shows functions list in "list" slide', async function () {
    ...
    const listView = listSlide.querySelector('.content-workflows-functions-list-view');
  ...
  it('has new function form in "creator" slide', async function () {
    ...
    expect(creatorSlide.querySelector('.content-workflows-functions-creator-view'))
      .to.exist;
  });
  ...
  it('allows to add new function', async function () {
    ...
    expect(createCreateLambdaFunctionActionStub).to.be.calledOnce
      .and.to.be.calledWith(sinon.match({
        // Checking only if workflowDirectory is passed. Function creation is tested
        // in nested components.
        workflowDirectory: this.get('workflowDirectory'),
      }));
    expect(isSlideActive('list')).to.be.true;
  });
...
async function render(testCase) {
  testCase.render(hbs `{{content-workflows-functions

...
describe('Integration | Component | content inventories workflows/loading view',
  function () {
    setupComponentTest('content-inventories-workflows/loading-view', {
      integration: true,
    });

    suppressRejections();

    it('has class "content-inventories-workflows-loading-view"', function () {
      this.render(hbs `{{content-inventories-workflows/loading-view}}`);

      expect(this.$().children()).to.have.class('content-inventories-workflows-loading-view')
...
async function render(testCase) {
  testCase.render(hbs `{{content-inventories-workflows/loading-view
...
function expectContentElementsCount(testCase, expectedContentElementsCount) {
  expect(testCase.$('.content-inventories-workflows-loading-view').children())
    // +1 for header

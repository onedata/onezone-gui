...
const CookiesConsentManagerStub = Service.extend({
...
describe('Integration | Component | cookies consent', function () {
  ...
  beforeEach(function () {
    registerService(this, 'cookiesConsentManager', CookiesConsentManagerStub);
  });
  ...
  it(
    ...
    function () {
      set(lookupService(this, 'cookiesConsentManager'), 'areCookiesAccepted', true);
      this.render(hbs `{{cookies-consent}}`);
  ...
  it('allows to accepts cookies', function () {
    ...
    return click('.accept-cookies').then(() => {
      expect(this.$('.cookies-consent')).to.not.exist;
      expect(
        get(lookupService(this, 'cookiesConsentManager'),
          'areCookiesAccepted')
      ).to.be.true;
    });
  });

...
describe('Integration | Component | content inventories workflows', function () {
  ...
  context('when "view" query param is "list"', function () {
    ...
    it('allows to open creator view', async function () {
      ...
      await click(getSlide('list').querySelector('.open-add-workflow-schema-trigger'));
    ...
    it('allows to open editor view for specific workflow schema', async function () {
      ...
      await click(getSlide('list').querySelector('.workflows-list-entry'));
...
function expectSlideContainsView(slideId, viewName) {
  ...
  expect(slide.children[0].className)
    .to.contain(`content-inventories-workflows-${viewName}-view`);
